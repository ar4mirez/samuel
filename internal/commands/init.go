...
func runInit(cmd *cobra.Command, args []string) error {
	...
	// Create per-folder CLAUDE.md stubs in existing directories
	if err := scanAndCreateFolderCLAUDEMD(absTargetDir); err != nil {
		ui.Warn("Could not create per-folder CLAUDE.md files: %v", err)
	}
...
// scanAndCreateFolderCLAUDEMD scans existing directories and creates CLAUDE.md stubs
func scanAndCreateFolderCLAUDEMD(rootDir string) error {
	// Directories to skip
	skipDirs := map[string]bool{
		".git": true, ".claude": true, ".agent": true,
	...
	for _, entry := range entries {
		...
		claudePath := filepath.Join(rootDir, name, "CLAUDE.md")
	...
	if created > 0 {
		ui.Success("Created %d per-folder CLAUDE.md stubs", created)
	}
	return nil
}
