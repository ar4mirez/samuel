...
```mermaid
graph TD
    A[aicof auto init] --> B[prd.json created]
    B --> C[aicof auto start]
    C --> D{Fresh Context}
...
The human can re-enter the loop at any point by stopping auto.sh and returning to manual task-by-task implementation.
