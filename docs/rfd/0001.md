...
This RFD documents the architectural decision to use a progressive disclosure pattern for AI system instructions in AICoF. Instead of a monolithic instruction file, the system uses a core CLAUDE.md (~280 lines of essential content) combined with on-demand context loading from the `.agent/` directory.
...
Core guardrails always loaded, detailed context loaded on-demand from `.agent/` directory.
...
- **Backwards compatibility**: Works without `.agent/` directory
...
- Decision documented: `.agent/memory/2025-01-15-claude-md-redesign.md`
