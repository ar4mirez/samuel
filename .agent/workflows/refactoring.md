...
> **Purpose**: Structured approach to technical debt remediation. Improve code structure while maintaining behavior through incremental, safe changes.
...
function validateOrder(order: Order) { /* 20 lines */ }
function calculateTotals(order: Order) { /* 25 lines */ }
function processPayment(order: Order, totals: Totals) { /* 30 lines */ }
function sendConfirmation(order: Order) { /* 20 lines */ }
```
...
function calculateShipping(order: Order) {
  switch (order.shippingType) {
    ...
    case 'overnight': return order.weight * 15 + 30;
  }
}
...
class ExpressShipping implements ShippingStrategy {
  calculate(order: Order) { return order.weight * 10 + 15; }
}

class OvernightShipping implements ShippingStrategy {
  calculate(order: Order) { return order.weight * 15 + 30; }
}
```
...
- [code-review.md](code-review.md) - Identifies refactoring needs
- [testing-strategy.md](testing-strategy.md) - Ensures safety net
- [troubleshooting.md](troubleshooting.md) - If refactoring breaks something
